...
For each sample AS events were quantified using the percentage of sequence inclusion (PSI) metric, which is a number ranging from zero and 100, corresponding to full sequence skipping or full sequence inclusion in all transcripts of a gene, respectively. For example, constitutively spliced exons have a PSI of 100. The PSI cannot be calculated for the very first and last exons of a gene.
...
Events with a *P*-value \<= `r pval_thrshld` and a \|∆PSI\| \>= `r dPSI_thrshld` were considered significant. The non parametric Mann-Whitney  (`rstatix::wilcox_test`) was also used to check if not assuming normality impacts the results drastically.
...
set.seed(16)
pheatmap(zMad(tidy_full_mat), cluster_rows = T, cluster_cols = T, 
...
The real data (in red) is inflated for zeros (full skipping) and 100 (full inclusion) which expected, however the PSI in the middle to resemble a normal distributio (in blue), in fact binning the data into 10 bins shows a more similar overlap

```{r, histrogram_PSI_binned_distribution_and_normally_distributed_PSI}
test_taf2_dat |>
  pivot_longer(cols = starts_with('PSI')) |>
  ggplot( aes(x = value, fill = name) ) +
...
Check the number of DSE in each condition. In the next plot I define if an event is differentially spliced in TAF2, NLS-TAF2∆IDR or both. The `GROUP` column defines direction of the DSE relative to control for TAF2 and NLS-TAF2∆IDR, respecitvely, meaning that if an event is not DSE in that condition it is labelled as "NO"

```{r, num_dse_diff_by_event_type}
num_dse_diff |>
...
ggplot() +
  ...
  geom_text(aes(label = Num_DSE, group = TYPE), size = 2,
...
```{r, DSE_plot_count, fig.width = 3, fig.height = 5, echo = F, eval = F}
# OLD CODE
ggplot(num_DSE) +
...
ggsave(filename = '99_Num_DSE_Bar.pdf', plot = last_plot(), 
       device = cairo_pdf, path = expr_dir, units = 'cm', width = 4,
       height = 7)

```

```{r, DSE_plot_count_stacked, fig.width = 5, fig.height = 1, echo = F, eval = F}
# OLD CODE
...
::: callout-note
The part in this report can be improved
:::
...
Export of each method both up and down DSE
