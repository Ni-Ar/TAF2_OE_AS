...
Alternative formulat to z-score that used median
...
```{r, get IDs of up and down exons cmpr method TAF2}
cmpr_events_mrgd |>
...
```{r, get IDs of up and down exons cmpr method TAF2dIDR}
cmpr_events_mrgd |>
...
```{r, get IDs of up and down exons tidy method TAF2}
subset(tidy_dPSI_pval, OE == 'TAF2') |>
  select(GENE, EVENT, dPSI_TAF2, p_TAF2, padj_TAF2, DSE_TAF2) |>
  subset(DSE_TAF2) |>
...
```{r, diff_TAF2_hist, fig.width = 6, fig.height = 2}
...
```{r, diff_TAF2_dIDR_hist, fig.width = 6, fig.height = 2}
...
cairo_pdf(file = file.path(plot_dir, '17_overlap_diff_exons_Upset.pdf'), 
...
cairo_pdf(file = file.path(plot_dir, '18_overlap_diff_introns_Upset.pdf'), 
          width = 4.1, height = 4, bg = NA)
intersection_introns_ids
dev.off()
```
...
EX_up_uniqTAF2dIDR <- EX_up_TAF2dIDR[!(EX_up_TAF2dIDR %in% c(EX_up_common, EX_up_TAF2, EX_do_TAF2, EX_do_TAF2dIDR))]
