...
Besides the different number of events the event-types trend is basically the same, except maybe for intron retentions. On the one hand, this reinsurances me that the super sample is a good representation of the replicates with just more coverage. 
Save bar plot.
...
The ∆PSI magnitudes are however somewhat comparable between the 2 OE (TAF2 on the left, NLS-TAF2∆IDR on the right). 
...
```{r, plot_some_events_both_both_kind, fig.width = 11, fig.height = 5}
...
```{r, plot_some_events_super_sample_but_not_in_rep_comparions, fig.width = 9, fig.height = 5}
...
A more interesting case could be the alternative acceptor [HsaALTA1003439-1/2](https://vastdb.crg.eu/event/HsaALTA1003439-1/2@hg38) of exon 94 of the SYNE2 gene which seems to be always skipped in basal condition and becomes a used splice site only upon OE.
...
```{r, plot_uniq_events, fig.width = 3, fig.height = 2}
...
```{r, get IDs of up and down exons and introns cmpr method TAF2}
cmpr_events_mrgd |>
...
```{r, get IDs of up and down a5 and a3 cmpr method TAF2}
...
```{r, get IDs of up and down exons and introns cmpr method TAF2dIDR}
cmpr_events_mrgd |>
...
```{r, get IDs of up and down a5 and a3 cmpr method TAF2dIDR}
...
```{r, select_coverage_and_make_heatmap, fig.width = 5, fig.height = 8}
tidy_events |> 
  left_join(y = mtdf, by = join_by(Sample) ) |>
...
The controls cluster well together but between the TAF2 and NLS-TAF2 overexpression the samples are all pretty similar. 
Now plot only the events that have full coverage in across all 9 samples.

```{r, heatmap_full_coverage_across_replicates, fig.width = 5, fig.height = 8}
tidy_events |> 
  left_join(y = mtdf, by = join_by(Sample) ) |>
  subset(Replicate != 'Merge' ) |>
  group_by(EVENT) |>
...
```{r, heatmap_zscores_super_sample, fig.width = 4, fig.height = 8}
tidy_events |> 
  left_join(y = mtdf, by = join_by(Sample) ) |>
  subset(Replicate == 'Merge' ) |>
  group_by(EVENT) |>
...
```{r, heatmap_zMad_super_sample, fig.width = 4, fig.height = 8}
...
```{r, heatmap_logit_transformation, fig.width = 4, fig.height = 8, evel = F}
...
```{r, psi_plot_logit, message = FALSE, fig.width = 11, fig.height = 5}
...
These are events that are not considered by the `compare` method, because they barely show a ∆PSI >= 15 however some of the events (but not all) in the plot above could be considered differentially spliced. For example, [HsaINT0126337](https://vastdb.crg.eu/event/HsaINT0126337@hg38) or [HsaALTD1053720-1/2](https://vastdb.crg.eu/event/HsaALTD1053720-1/2@hg38) might be worth considering. In the next step of the analysis I'll try to select such type of events using a more robust statistical method, rather than just ranking the by a scaled value.
...
```{r, histrogram_PSI_binned, fig.width = 6, fig.height = 4}
test_taf2_dat |>
...
```{r, histrogram_PSI_binned_distribution_and_normally_distributed_PSI, fig.width = 6, fig.height = 4}
test_taf2_dat |>
  pivot_longer(cols = starts_with('PSI')) |>
  ggplot( aes(x = value, fill = name) ) +
...
```{r, qq_plot, fig.width = 6, fig.height = 4}
test_taf2_dat |>
...
```{r, check_pvalue_distribution, fig.width = 6, fig.height = 4}
tidy_dPSI_pval |>
...
```{r, apply statistical thresholds}
dPSI_squish <- 40
...
```{r, volcano_plot_TAF2, fig.width = 5, fig.height = 6, warning = FALSE, message = FALSE}
...
```{r, select_stat_TAF2, message = TRUE, fig.height=5.8, fig.width=10}
stat_TAF2 <- unique(subset(plot_TAF2, DSE_TAF2)$EVENT)
message(length(stat_TAF2), ' significant events with low ∆PSI in TAF2 OE')
```

```{r, get IDs of up and down exons and introns tidy method TAF2}
subset(tidy_dPSI_pval, OE == 'TAF2') |>
  select(GENE, EVENT, dPSI_TAF2, p_TAF2, padj_TAF2, DSE_TAF2) |>
  subset(DSE_TAF2) |>
...
```{r, get IDs of up and down alt donor and acceptor tidy method TAF2}
...
```{r, volcano_plot_TAF2_dIDR, fig.width = 5, fig.height = 6, warning = FALSE, message = FALSE}
...
```{r, select_stat_TAF2dIDR, message = TRUE, fig.height = 5.5, fig.width = 9}
...
```{r, get IDs of up and down exons tidy method TAF2dIDR}
subset(tidy_dPSI_pval, OE == 'NLSTAF2dIDR') |>
  select(GENE, EVENT, dPSI_TAF2dIDR, p_TAF2dIDR, padj_TAF2dIDR, DSE_TAF2dIDR) |>
  subset(DSE_TAF2dIDR) |>
...
```{r, get IDs of up and down alt donor and acceptor tidy method TAF2dIDR}
...
```{r, show example found in compare2}
...
```{r, diff_TAF2_hist, fig.width = 6, fig.height = 3}
...
```{r, diff_TAF2_dIDR_hist, fig.width = 6, fig.height = 3}
...
```{r, tell about getting IDs details of TAF2dIDR}
...
```{r, num_dse_diff_by_event_type, fig.width = 7, fig.height = 6}
num_dse_diff |>
...
```{r, get IDs of up and down exons diff method TAF2}
...
```{r, get IDs of up and down alt donor and acceptor diff method TAF2}
...
```{r, get IDs of up and down exons diff method TAF2dIDR}
...
```{r, get IDs of up and down alt donor and acceptor diff method TAF2dIDR}
...
```{r, show_upset_plot_all_ex, fig.width = 4.1, fig.height = 4}
intersection_exons_ids
```

```{r, save upset plot exons interactions, message = FALSE, warning = FALSE}
...
```{r, show_upset_plot_all3, fig.width = 4.1, fig.height = 4}
...
```{r, make list altDA for upset plot}
...
```{r, make upset plot all alternative splice sites events}
...
```{r, all ss taf2 didr, fig.width = 6.1, fig.height = 5}
intersection_ss_ids
```
...
```{r, show_upset_plot_all2, fig.width = 7, fig.height = 5.5}
...
```{r, show_upset_plot_all_with_mrgd, fig.width = 10, fig.height = 6}
...
```{r, make dataframe of shared and unique exons}
rbind(
...
```
```{r, make dataframe of shared and unique introns}
rbind(
  data.frame(EXPERIMENT = 'TAF2',
             TYPE = "Intron",
...
```
```{r, make dataframe of shared and unique alternative splice sites}
rbind(
  data.frame(EXPERIMENT = 'TAF2',
             TYPE = "Alt. 3' ss",
...
```{r, export list of differentially spliced events}
...
```{r, tell about getting IDs details of all AS events}
...
```{r, export dataframe of the annotated differentially spliced events with dPSI}
...
```{r, generate supplementary table TAF}
Supp_Tbl_TAF <- 
...
```{r, export a supplementary table TAF}
...
In total TAF2 and NLS-∆IDR-TAF2 over-expression result in `r unique(SHR_UNI_AS_events$EVENT)` differentially spliced events, that are either shared between the 2 experiments or unique to each over-expression.
